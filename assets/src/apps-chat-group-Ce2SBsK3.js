import { a as e } from "../admin.bundle-CBXoUBAg.js";
import "../main-B7Jkv9i9.js";
import { t } from "../sweetalert2-BUm3ePnk.js";
/* empty css                            */ var n = e(t(), 1);
VirtualSelect.init({
  ele: `#addMemberSelect`,
  options: [
...
var r = [
    ...
    },
    {
      ...
      message: `Hello, How are you?`,
      time: `04:00PM`,
      img: `assets/images/brands/img-03.png`,
      badge: null,
      active: !1,
    },
    {
      ...
      message: `Hello, How are you?`,
      time: `05:14PM`,
      img: `assets/images/brands/img-04.png`,
      badge: null,
      active: !1,
    },
    {
      ...
      message: `Hello, How are you?`,
      time: `11:57AM`,
      img: `assets/images/brands/img-05.png`,
      badge: null,
      active: !1,
    },
    {
      ...
      message: `Can you review the latest UI changes?`,
      time: `09:42AM`,
      img: `assets/images/brands/img-06.png`,
      badge: 2,
      active: !1,
    },
    {
      ...
      time: `10:18AM`,
      img: `assets/images/brands/img-07.png`,
      badge: null,
      active: !1,
    },
    {
    ...
    {
      id: 9,
      name: `Product Thinkers`,
      message: `Let’s finalize the roadmap today.`,
      time: `Yesterday`,
      img: `assets/images/brands/img-09.png`,
      badge: null,
      active: !1,
    },
    {
      id: 10,
      name: `QA Heroes`,
      message: `Found a few edge cases in checkout.`,
      time: `08:36AM`,
      img: `assets/images/brands/img-10.png`,
      badge: 3,
      active: !1,
    },
  ],
  i = document.getElementById(`chatList`);
r.forEach((e) => {
  let t = e.badge
      ? `<span class="badge bg-danger-subtle text-danger">${e.badge}</span>`
      : ``,
    n = e.active ? `active` : ``;
  i.innerHTML += `
      <li id="chat-${e.id}">
...
var a = document.querySelectorAll(`.chat-list-item`);
a.forEach((e) => {
  e.addEventListener(`click`, (t) => {
    a.forEach((e) => e.classList.remove(`active`)), e.classList.add(`active`);
    let n = e.closest(`li`).id.split(`-`)[1];
    r.forEach((e) => {
      e.active = e.id == n;
    });
  });
});
var o = document.getElementById(`saveGroupBtn`),
  s = document.getElementById(`locationInput`);
o.addEventListener(`click`, () => {
  ...
  if (e !== ``) {
    let t = {
      ...
      img: `assets/images/brands/img-27.png`,
      badge: null,
      active: !1,
    };
    r.unshift(t);
    ...
    i.insertAdjacentHTML(`afterbegin`, n),
      (s.value = ``),
      window.bootstrap.Modal.getInstance(document.querySelector(`.modal`));
  } else alert(`Please enter group name`);
});
var c = document.getElementById(`searchChat`),
  l = document.getElementById(`noResult`);
c.addEventListener(`keyup`, () => {
  let e = c.value.toLowerCase(),
    t = document.querySelectorAll(`#chatList li`),
    n = !1;
  t.forEach((t) => {
    t.querySelector(`h6`).textContent.toLowerCase().includes(e)
      ? ((t.style.display = `block`), (n = !0))
      : (t.style.display = `none`);
  }),
    n ? l.classList.add(`d-none`) : l.classList.remove(`d-none`);
});
var u = document.querySelector(`.avatar img`),
  d = document.querySelector(`.avatar`).nextElementSibling.querySelector(`a`),
  f = document.querySelector(`.text-center img`),
  p = document.querySelector(`.text-center h6`);
i.addEventListener(`click`, (e) => {
  let t = e.target.closest(`.chat-list-item`);
  if (!t) return;
  let n = t.querySelector(`img`).src,
    r = t.querySelector(`h6`).textContent;
  (u.src = n), (d.textContent = r), (f.src = n), (p.textContent = r);
});
var m = [
    {
      id: 1,
  ...
  h = document.querySelector(`.d-flex.flex-column.gap-5`);
document.querySelector(`.form-control`);
var g = document.querySelector(`.btn-active-primary`);
function _() {
  (h.innerHTML = ``),
    m.forEach((e) => {
      let t = `
      <div class="d-flex align-items-end gap-4 ${
        e.position === `right` ? `ms-auto` : ``
      } max-w-xl" data-id="${e.id}">
        ${
          e.position === `left`
            ? `
          ...
          <div class="d-flex align-items-end gap-4">
            <div class="flex-grow-1">
              <p class="text-muted mb-1 fs-12 ${
                e.position === `right` ? `text-end` : ``
              }">${e.time}</p>
              <div class="px-4 py-10px position-relative text-body ${
                e.position === `right`
                  ? `rounded-top-3 rounded-start-3 bg-light chat-right-bubble`
                  : `rounded-top-3 rounded-end-3 border chat-left-bubble `
              }">
                ${e.message}
              </div>
            </div>
    ...
    document.querySelectorAll(`.delete-msg`).forEach((e) => {
      e.addEventListener(`click`, function () {
        v(parseInt(this.dataset.id));
      });
    });
}
function v(e) {
  m.splice(
    m.findIndex((t) => t.id === e),
    1
  ),
    _();
}
function y() {
  let e = messageInput.value.trim();
  if (e !== ``) {
    let t = {
      id: m.length + 1,
      sender: `You`,
...
g.onclick = y;
var b = document.querySelector(`.chat-messages`);
function x() {
  setTimeout(() => {
...
messageInput.addEventListener(`keydown`, function (e) {
  e.key === `Enter` && !e.shiftKey && (y(), e.preventDefault());
}),
  document.addEventListener(`click`, function (e) {
    let t = e.target.closest(`.dropdown-item`);
    t &&
      t.innerText.includes(`Clear Chat`) &&
      n.default
        .fire({
          title: `Are you sure?`,
          text: `You want to clear all chat messages!`,
          icon: `warning`,
          showCancelButton: !0,
        ...
        .then((e) => {
          e.isConfirmed &&
            ...
            n.default.fire({
              title: `Cleared!`,
              text: `All chat messages have been cleared.`,
              icon: `success`,
              timer: 1500,
              showConfirmButton: !1,
            }));
